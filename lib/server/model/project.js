...
export default {
    ...
    async validProject ({ projectCode, projectName, fromOpenApi = false }) {
        ...
        let errMsg = fromOpenApi ? 'lesscode平台校验不通过：' : ''
        let code = ''
        ...
        if (fromOpenApi) {
            if (!openApiCodeRegex.test(projectCode)) {
                status = 400
                errMsg += '应用ID由开发者中心的"应用ID+模块名称"组成，只能包含小写字母, 32个字符以内'
                code = CODE.BIZ.PROJECT_ID_INVALID
            }
            if (!openApiNameRegex.test(projectName)) {
                status = 400
                errMsg += '应用名称由开发者中心的"应用名称+模块名称"组成,只能包含汉字，英文字母，数字，36个字符以内'
                code = CODE.BIZ.PROJECT_NAME_EXISTED
            }
        } else {
            if (!codeRegex.test(projectCode)) {
                status = 400
                errMsg += '应用ID只能由小写字母组成, 16个字符以内'
                code = CODE.BIZ.PROJECT_ID_INVALID
            }
            if (!nameRegex.test(projectName)) {
                status = 400
                errMsg += '应用名称由汉字，英文字母，数字组成，20个字符以内'
                code = CODE.BIZ.PROJECT_NAME_EXISTED
            }
        }
