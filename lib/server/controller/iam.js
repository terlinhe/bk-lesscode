...
const iamController = {
    async serviceCheckAppPerm (ctx) {
        const params = ctx.request.body
        const actions = params.actions || undefined
        const bkToken = ctx.request.headers[global.AUTH_NAME] || params[global.AUTH_NAME]
        const response = await getUserInfoByToken(bkToken)
        let username = ''

        try {
            const { data } = response?.data
            username = data?.username || data?.bk_username
            if (!username) {
                ctx.throwError({
                    message: '获取用户名失败'
                })
                ...
                const iamRes = await iamController.checkInServer(ctx, String(params.projectId), actions)
                if (iamRes[IAM_ACTION.develop_app[0]] === true) {
                    ctx.send({
                        code: 0,
                        message: 'OK',
                        data: {
    ...
    async myProject (ctx) {
        try {
            ...
            query.condition.push('archiveFlag = 0')
    ...
    async check (ctx) {
        ...
        if (!global.IAM_ENABLE || noPermActions.includes(action)) {
    ...
    async checkInServer (ctx, resourceId, actions = [
        ...
        if (!global.IAM_ENABLE) {
            // 再check一次， 用户是否属于当前应用
            const project = await projectModel.findUserProjectById(ctx.session?.userInfo?.id, resourceId)
            if (project) {
                return {
                    [IAM_ACTION.develop_app[0]]: true,
                    [IAM_ACTION.deploy_app[0]]: true,
                ...
                return {
                    [IAM_ACTION.develop_app[0]]: false,
                    [IAM_ACTION.deploy_app[0]]: false,
                    [IAM_ACTION.manage_perms_in_app[0]]: false,
                    [IAM_ACTION.manage_app[0]]: false
                }
            }
