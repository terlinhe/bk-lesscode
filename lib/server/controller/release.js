...
import { getReleaseInfo, uploadToBkRepo, uploadToV3, deploy, getAuthHeader, getAuthHeaderWithoutUser } from '../service/business/v3-service'
...
import { getTenantIdHeader } from '../service/business/v3-service'
import {
    execApiGateWay
} from '@bkui/apigateway-nodejs-sdk'
import { iamAppPermOperate } from './iam'
import { createAppBkFlowSpace } from './flow'
...
const Realease = {
    ...
    async applicationList(ctx) {
        try {
            if (!config.URL_PREFIX) {
                ctx.throwError({
                    message: global.i18n.t('请先配置paas平台相关信息')
                })
            }
            const url = `${config.URL_PREFIX}/bkapps/applications/lists/minimal?include_inactive=true`
    ...
    async moduleList(ctx) {
        try {
            ...
            const url = `${config.URL_PREFIX}/bkapps/applications/${appCode}/modules/?source_origin=2`
            const headers = getAuthHeader(ctx.cookies.get(global.AUTH_NAME))
            const res = await ctx.http.get(url, { headers })
            ...
            if (err.message === 'Request failed with status code 403') {
                err.message = global.i18n.t('无权限操作当前应用模块')
            }
            if (err.message === 'Request failed with status code 404') {
                err.message = global.i18n.t('获取模块列表失败')
            }
    ...
    async detailInfo(ctx) {
        try {
            ...
            const { bindInfo } = ctx.request.body
    ...
    async releaseProject(ctx) {
        ...
        releaseForm.projectId = parseInt(ctx.request.headers['x-project-id'], 10)

        // 非历史版本部署（应用版本）
        const isNewPackage = releaseForm.releaseType !== 'HISTORY_VERSION'

        let validMsg = ''
        if (!releaseForm.version) validMsg = global.i18n.t('版本号不能为空')
        ...
        if (isNewPackage) {
            ...
            if (isExist && isExist.id) {
                const result = isExist.status === 'successful' ? global.i18n.t('成功') : (isExist.status === 'failed' ? global.i18n.t('失败') : isExist.status)
                validMsg = global.i18n.t('该版本号已存在，部署结果为{{0}}，请重新输入', { n: result })
            }
            ...
            if (releaseForm.isCreateProjVersion) {
                ...
                if (isExist) {
                    validMsg = global.i18n.t('该版本号已存在于应用版本，请重新输入')
                }
            }
        }

        const project = await ProjectModel.findProjectDetail({ id: releaseForm.projectId }) || {}
        if (!project.appCode || !project.moduleCode) validMsg = global.i18n.t('请先绑定蓝鲸应用及相应模块')
        ...
        if (!id) {
            ctx.throwError({
                message: global.i18n.t('创建部署记录失败')
            })
        }
        ...
        try {
            ...
            await Promise.all(flowList.map(async flow => {
                const { result, data } = await execApiGateWay({
                    ...
                    config: { headers: getTenantIdHeader() }
                })
                if (result) {
            ...
            try {
                // 为部署应用创建bkflow空间
                const bkflowSpaceName = process.env.NODE_ENV === 'production' ? `lesscode_app_${releaseForm.projectId}` : `lesscode_app_${releaseForm.projectId}_dev`
                await createAppBkFlowSpace({
                    projectId: releaseForm.projectId,
                    name: bkflowSpaceName,
                    appCode: releaseForm.appCode,
                    superusers: [project.createUser]
                })
            } catch (error) {
                logger.error('createAppBkFlowSpace error', error)
            }
            ...
            if (isNewPackage) {
                ...
                // 上传包到文件仓库并签名、计算md5
                const { codeUrl, md5 } = await uploadToBkRepo(releaseForm.projectId, releaseForm.version, releaseForm.versionId, id)
                await ReleaseModel.updateVersion(id, { codeUrl: codeUrl, releaseMd5: md5 })
                console.log(codeUrl, 'codeurl')

                // 同步包到v3
                Object.assign(appInfo, { createUser: project.createUser })
            ...
            if (releaseForm.releaseSqlIds) {
                await http.request({
                    ...
                    method: 'post',
                    headers: getAuthHeaderWithoutUser()
                })
            }
            ...
            try {
                ...
                await execApiGateWay({
                    ...
                    data: {
                        apps: [appInfo.appCode],
                        permissions: ['create_space', 'create_task_without_template', 'get_task_detail', 'get_task_states', 'get_tasks_states', 'operate_task', 'operate_task_node']
                    },
                ...
                logger.error('release itsm flow error', error)
            }
    ...
    async getList(ctx) {
        try {
            const projectId = ctx.request.headers['x-project-id']
            let res = await ReleaseModel.getList(projectId)
            res = res.map(item => {
                const { codeUrl, ...others } = item
                return others
            })
            ctx.send({
                code: 0,
    ...
    async getAppDetail(ctx) {
        try {
            const { appCode } = ctx.request.query
            const url = `${config.URL_PREFIX}/system/uni_applications/query/by_id/?id=${appCode}`
            const headers = getAuthHeaderWithoutUser()
            const res = await ctx.http.get(url, { headers })
    ...
    async getRunningLog(ctx) {
        try {
            ...
            const url = `${config.URL_PREFIX}/streams/${deployId}/history_events`
            const headers = getAuthHeader(ctx.cookies.get(global.AUTH_NAME))
            const res = await ctx.http.get(url, { headers }) || []
    ...
    async detailFromV3(ctx) {
        try {
            ...
            const { deployId } = ctx.request.query
            const project = await ProjectModel.findProjectDetail({ id: projectId }) || {}
            const url = `${config.URL_PREFIX}/bkapps/applications/${project.appCode}/modules/${project.moduleCode}/deployments/${deployId}/result/`
            const headers = getAuthHeader(ctx.cookies.get(global.AUTH_NAME))
            const res = await ctx.http.get(url, { headers })
    ...
    async checkAppInfoExist(ctx) {
        ...
        if (projects && projects.length) {
            ctx.throwError({
                message: global.i18n.t('该蓝鲸应用模块已被其它应用绑定，请重新选择')
            })
        } else {
            ctx.send({
                code: 0,
    ...
    async offlineProject(ctx) {
        try {
            let validMsg = ''
            const projectId = ctx.request.headers['x-project-id']
            const { env, version } = ctx.request.body
            const project = await ProjectModel.findProjectDetail({ id: projectId }) || {}
            if (!project.appCode || !project.moduleCode) validMsg = global.i18n.t('请先绑定蓝鲸应用及相应模块')
            validMsg && ctx.throwError({
                message: validMsg
    ...
    async checkOfflineResult(ctx) {
        try {
            const projectId = ctx.request.headers['x-project-id']
            const { offlineId } = ctx.request.query
            const project = await ProjectModel.findProjectDetail({ id: projectId }) || {}

            const url = `${config.URL_PREFIX}/bkapps/applications/${project.appCode}/modules/${project.moduleCode}/offlines/${offlineId}/result/`
            const headers = getAuthHeader(ctx.cookies.get(global.AUTH_NAME))
            const res = await ctx.http.get(url, { headers })
    ...
    async getDeployPhasesData(ctx) {
        try {
            ...
            let url = `${config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/envs/${env}/deploy_phases/`
            if (deployId) {
                url = `${config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/envs/${env}/deploy_phases/${deployId}`
            }
            const headers = getAuthHeader(ctx.cookies.get(global.AUTH_NAME))
            const res = await ctx.http.get(url, { headers }) || []
            ctx.send({
    ...
    async getLastVersionProcesses(ctx) {
        try {
            ...
            const url = `${config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/envs/${env}/processes/list/`
