...
import QueryRecord from '../model/entities/query-record'
    ...
    try {
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            const categoryTaskList = projectList.map((project) => {
                if (!categoryList.find(category => category.name === '默认分类' && category.projectId === project.id)) {
...
async function initPageTemplateAndFuncMarket () {
    try {
        ...
        return {
            code: 0,
            message: '初始化页面模板和函数市场成功'
        }
    } catch (error) {
        return {
            code: -1,
            message: `初始化页面模板和函数市场失败: ${error}`,
            data: null
        }
    }
}
...
async function updateFormTableName () {
    try {
        ...
        return {
            code: 0,
            message: 'form表tableName更新成功'
        }
    } catch (error) {
        return {
            code: -1,
            message: `form表tableName更新失败: ${error}`,
            data: null
        }
    }
}
...
async function updateLayoutThemeData () {
    try {
        ...
        return {
            code: 0,
            message: 'layoutInst表themeConfig更新成功'
        }
    } catch (err) {
...
// eslint-disable-next-line no-unused-vars
async function transformDataSourceType () {
    try {
        const pageRepository = getRepository(Page)
        const pageList = await pageRepository.find()
        ...
        const QueryRecordRepository = getRepository(QueryRecord)
        const queryRecordList = await QueryRecordRepository.find()
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            const queryRecordTask = queryRecordList.map((queryRecord) => {
                return transactionalEntityManager.update(QueryRecord, {
                    id: queryRecord.id
                }, {
                    dataSourceType: 'preview',
                    updateTime: queryRecord.updateTime,
                    updateUser: queryRecord.updateUser
                })
            })
            await Promise.all([...taskList, ...tempTaskList, ...queryRecordTask])
        })
        return {
            code: 0,
            message: `transformDataSourceType success：${(new Date()).toString()}`,
            data: null
        }
    } catch (error) {
        console.dir(error)
