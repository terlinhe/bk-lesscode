...
export default class OpenApiController {
    ...
    async filterTableDataWithConditions (
        ...
        // 获取数据
        return filterTableDataWithConditions(conditions, tableName, group, fields)
    }
    ...
    async executeApi (
        ...
        try {
            ...
            if (tableNameParam && tableOperateHandler) {
                ...
                console.log('-----------> ctx', tableNameParam, tableOperateHandler, apiData, creatorUsername)
                await dataSourceController.prototype[tableOperateHandler](ctx)
            } else {
                throw new global.BusinessError('api调用失败', -1, 500)
            }
        } catch (e) {
            throw new global.BusinessError(e.message || e, -1, 500)
        }
    }
