...
import { getAuthHeaderWithoutUser } from '../business/v3-service'
...
export default class OnlineService {
    ...
    // 展示表详情
    async describeTables (tableNames, database) {
        const nameSqlFilter = tableNames.join('\',\'')
        const columnSql = `
            SELECT * FROM INFORMATION_SCHEMA.TABLES t
            LEFT JOIN INFORMATION_SCHEMA.COLUMNS c
            ON t.TABLE_NAME = c.TABLE_NAME
            WHERE t.TABLE_NAME IN ('${nameSqlFilter}') AND t.TABLE_SCHEMA = '${database}' AND c.TABLE_SCHEMA = '${database}'
        ...
        const tableList = columnsList.reduce((acc, cur) => {
            ...
            const column = {
                ...
                unique: COLUMN_KEY === 'UNI' || COLUMN_KEY === 'PRI',
                nullable: IS_NULLABLE !== 'NO',
                default: COLUMN_DEFAULT,
                comment: COLUMN_COMMENT,
                length: normalizeColumnLength(),
                scale: NUMERIC_SCALE,
                generated: EXTRA === 'auto_increment'
            }

            if (DATA_TYPE === 'enum') {
                const execResult = /\((.+)\)/.exec(COLUMN_TYPE)
                if (execResult) {
                    const enumValues = execResult[1].split(',')
                    column.enum = enumValues.map(val => val.slice(1, -1))
                }
            }
    ...
    // 分页获取表数据
    async getTableData (tableName, page, pageSize, filterKey, filterValue, sortKey, sortValue) {
        const where = `${filterKey && filterValue ? `WHERE \`${filterKey}\` LIKE '%${filterValue}%'` : ''}`
        const orderBy = `ORDER BY ${sortKey || 'id'} ${sortValue || 'DESC'}`
        const [list, foundRows = []] = await this.dbEngine.execMultSql(`
            SELECT SQL_CALC_FOUND_ROWS * FROM \`${tableName}\` ${where} ${orderBy} LIMIT ${(page - 1) * pageSize},${pageSize};
            SELECT FOUND_ROWS();
        `)
    ...
    // 获取表所有的数据
    async getTableAllData (tableName, filterKey, filterValue, sortKey, sortValue) {
        const where = `${filterKey && filterValue ? `WHERE \`${filterKey}\` LIKE '%${filterValue}%'` : ''}`
        const orderBy = `ORDER BY ${sortKey || 'id'} ${sortValue || 'DESC'}`
        const [list, foundRows = []] = await this.dbEngine.execMultSql(`
            SELECT SQL_CALC_FOUND_ROWS * FROM  \`${tableName}\` ${where} ${orderBy};
            SELECT FOUND_ROWS();
        `)
        const rows = foundRows[0] || {}
        const count = Object.values(rows)[0] || 0
