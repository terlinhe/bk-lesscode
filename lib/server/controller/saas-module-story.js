...
export default class SaasModuleStoryController {
    ...
    async createStory (
        ...
        // 写入操作记录
        await LCDataService.add(TABLE_FILE_NAME.SAAS_OPERATE_LOG, {
            projectId,
            moduleId: params.moduleId,
            operateType: 'ADD_STORY',
            status: '',
            payloadData: JSON.stringify({ con: params.story })
        })
    ...
    async patchStory (
        ...
        // 写入操作记录
        const { operateData } = params
        if (operateData?.payloadData) {
            Object.assign(operateData, {
                status: '',
                projectId
            })
            await LCDataService.add(TABLE_FILE_NAME.SAAS_OPERATE_LOG, operateData)
        }
