...
async function initSystem () {
    return sendReq(
        ...
        {
            id: IAM_APP_ID,
            name: global.i18n.t('蓝鲸可视化开发平台'),
            name_en: 'lesscode',
            description: global.i18n.t('蓝鲸可视化开发平台提供了前端页面在线可视化拖拽组装、配置编辑、源码生成、二次开发等能力。旨在帮助用户通过尽量少的手写代码的方式快速设计和开发 SaaS。'),
...
async function updateSystem () {
    return sendReq(
        ...
        {
            name: global.i18n.t('蓝鲸可视化开发平台_0'),
            name_en: 'lesscode',
            description: global.i18n.t('蓝鲸可视化开发平台提供了前端页面在线可视化拖拽组装、配置编辑、源码生成、二次开发等能力。旨在帮助用户通过尽量少的手写代码的方式快速设计和开发 SaaS。_0'),
            description_en: 'BlueKing LessCode provides the front-end page online visual drag-and-drop assembly, configuration editing, source code generation, secondary development and other capabilities. Designed to help users quickly design and develop SaaS with as little handwritten code as possible.',
            clients: 'lesscode',
            provider_config: {
                host: IAM_PROVIDER_HOST,
...
async function initPlatformActionPerm2 () {
    try {
        const params = {
            groups: [
                {
                    name: 'platform-manage2',
                    description: '具有管理平台公开模板、函数、查看统计数据等平台级别权限'
                }
            ]
        }
        const a = sendReq(
            `${IAM_HOST}/api/v2/open/management/systems/${IAM_APP_ID}/grade_managers/-/groups/`,
            params,
            'post',
            'addgroupuser'
        )
        ...
        const groupRes = await http.post(`${IAM_HOST}/api/v2/open/management/systems/${IAM_APP_ID}/grade_managers/-/groups/`, params, {
            headers: {
                'X-Bkapi-Authorization': JSON.stringify({
                    bk_app_code: IAM_APP_ID,
                    bk_app_secret: IAM_APP_SECRET
        ...
        let platformGroupId = 0
        if (groupRes === 0) {
            platformGroupId = groupRes.data[0]
            ...
            const memberParams = {
                ...
                expired_at: 4848453381
            }
            const memberRes = await http.post(`${IAM_HOST}/api/v2/open/management/systems/${IAM_APP_ID}/groups/${platformGroupId}/members/`, memberParams, {
                headers: {
                    'X-Bkapi-Authorization': JSON.stringify({
                        bk_app_code: IAM_APP_ID,
                        bk_app_secret: IAM_APP_SECRET
            ...
            const grantRes = await http.post(`${IAM_HOST}/api/v2/open/management/systems/${IAM_APP_ID}/groups/${platformGroupId}/policies/`, grantParams, {
                headers: {
                    'X-Bkapi-Authorization': JSON.stringify({
                        bk_app_code: IAM_APP_ID,
                        bk_app_secret: IAM_APP_SECRET
        ...
        if (platformGroupId) {
            return {
                code: 0,
                message: 'initPlatformActionPerm: Insert success'
            }
        } else {
            return {
                code: 0,
        ...
        return {
            code: -1,
            message: `initPlatformActionPerm: ${err.message || err}`
        }
    }
}
