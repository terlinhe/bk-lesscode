...
export const getTenantIdHeader = () => {
    return {
        'X-Bk-Tenant-Id': global.ENV_ENABLE_MULTI_TENANT ? (global.TENANT_ID || 'default') : undefined
    }
}
...
export const getReleaseInfo = async (projectId, appCode, moduleCode, env, bkTicket, bindInfo) => {
    const url = `${v3Config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/envs/${env}/released_state/`
    const headers = getAuthHeader(bkTicket)
    const options = {
        url,
        headers
    }
...
export const getAppInfo = async (appCode, bkTicket) => {
    const url = `${v3Config.URL_PREFIX}/bkapps/applications/${appCode}/`
    const headers = getAuthHeader(bkTicket)
    const options = {
        url,
        headers
...
export const getModuleInfo = async (appCode, moduleCode, bkTicket, appUser) => {
    const url = `${v3Config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/`
    const headers = getAuthHeader(bkTicket)
    const options = {
        url,
        headers
...
export const uploadToV3 = async (releaseForm, appInfo, codeUrl, bkTicket) => {
    ...
    return new Promise((resolve, reject) => {
        request.post({ url, formData, headers }, function (error, response, body) {
...
export const deploy = async (releaseForm, appInfo, bkTicket) => {
    ...
    const headers = getAuthHeader(bkTicket)
    return new Promise((resolve, reject) => {
        request.post({ url, json: true, body: data, headers }, async function (error, response, body) {
...
export const createV3App = async (appData, bkTicket) => {
    ...
    const data = {
        ...
        app_tenant_mode: appData.tenantMode || undefined
    }
